...
import { User, Globe, Landmark, ShieldCheck } from "lucide-react"
...
export default function Booking() {
  ...
  useEffect(() => {
    if (passengerForm) {
      setName(passengerForm.name || "")
      setPassport(passengerForm.passport_no || "")
      setNationality(passengerForm.nationality || "")
    }
  }, [passengerForm])
  ...
  async function saveBooking() {
    ...
    // 🔥 FIX 2: safe values
    const flightPrice = selectedFlight?.price ?? 5000
  ...
  return (
    <div className="min-h-screen w-full flex items-center justify-center p-4 md:p-10 bg-cover bg-center"
      style={{
        backgroundImage:
          `linear-gradient(to bottom, rgba(224,242,254,0.4), rgba(224,242,254,0.2)),
           url('https://images.unsplash.com/photo-1524661135-423995f22d0b?q=80&w=1920&auto=format&fit=crop')`
      }}
    >
      ...
      <div className="w-full max-w-md backdrop-blur-xl bg-white/75 rounded-3xl p-8 shadow-2xl">
        ...
        <div className="bg-slate-100 p-4 rounded-xl mt-6 text-sm">
          <p>Flight: {selectedFlight?.flight_no}</p>
          <p>Seat: {selectedSeat || "None"}</p>
